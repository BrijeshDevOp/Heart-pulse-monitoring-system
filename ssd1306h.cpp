...
bool SSD1306::nextPage(void) {
    writePage(currentPage);          // Write current page buffer to display
    memset(pageBuf, 0x00, COLUMNS);  // Clear page buffer for next page
    currentPage++;                   // Advance to next page
    return currentPage < PAGES;      // Return false if all pages done
}

void SSD1306::drawNum(int x, int y, int num, const int BIG) {
    char buf[12];
